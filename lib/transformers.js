...
const { fieldParser, lineParser } = require('./parsers');

const NEWLINE = 0x0A;

/**
 * Multi-field input transformer. Takes as input a stream of field data,
 * with each field separated by a separator character, and process it
 * using the op function before writing the result to an output stream.
 ...
 * processed.
 */
async function fieldTransformer( ins, outs, op, ns, name, vars, opts = {} ) {
    ...
    // Process the input.
    await fieldParser( ins, process, separator );
}
...
async function lineTransformer( ins, outs, op, ns, name, vars, multiValue = false ) {
    ...
    return fieldTransformer( ins, outs, op, ns, name, vars, { multiValue, separator });
}
...
module.exports = { fieldTransformer, lineTransformer, jsonlTransformer };
